...
# Check arguments
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <project_name> <project_path> <output-file>"
    exit 1
fi
...
project_path=$2
output_file=$3
output_base="/home/siam/Desktop/volume1/MS_Papers_Arif/ChangeCode/change_probneness_relation_analysis/projects_analysis_results_two"
csv_file="$output_base/$project_name/$output_file"
...
# Get list of Python files that have been modified
changed_files=$(git log --name-only -l0 --pretty=format: $start_hash..HEAD | grep "\.py$" | sort | uniq)
...
# Function to calculate insertions and deletions
calculate_insertions_deletions() {
    local file=$1
    local start_commit=$2
    local end_commit=$3
...
for file in $changed_files; do
    ...
    # Get file creation hash
    creation_hash=$(git log --find-renames --diff-filter=A -- "$file" | head -n 1 | cut -d ' ' -f 2)
    if [ -z "$creation_hash" ]; then
        creation_hash=$start_hash
    fi
    
    # Calculate actual changes to the file (number of commits that modified the file)
    changes=$(git log --follow --format="%H" -- "$file" | wc -l)
    
    # Calculate total commits after file creation
